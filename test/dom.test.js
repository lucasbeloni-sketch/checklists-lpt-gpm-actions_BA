...
test("Tipo de Checklist so e selecionavel depois da Finalidade (AJAX)", { skip: !temChromium }, async () => {
  ...
  try {
    ...
    await assert.rejects(
      ...
      /nao selecionou "LPT - Visita Prévia-BA"/
    );
...
test("tipo: escolhe a LPT e nao as outras 4 opcoes com 'Visita Prévia'", { skip: !temChromium }, async () => {
  // Regressao do run 31422888135 (repo UTD): o token "Visita Pr" filtrava 5
  // itens e o Enter pegava o primeiro. Aqui o alvo e a LPT, mas o mecanismo e o
  // mesmo: clicamos no item de texto exatamente igual ao alvo, nunca no primeiro.
  const { browser, page } = await abrir();
  try {
    await selecionarChoices(page, CFG, "finalidade", CFG.finalidade, CFG.finalidadeSearch);
    await esperarTiposCarregar(page, CFG);
...
test("tipo: mesmo com token ambiguo ('Visita Pr') acerta a LPT pelo texto exato", { skip: !temChromium }, async () => {
  const { browser, page } = await abrir();
  try {
    await selecionarChoices(page, CFG, "finalidade", CFG.finalidade, CFG.finalidadeSearch);
    await esperarTiposCarregar(page, CFG);
