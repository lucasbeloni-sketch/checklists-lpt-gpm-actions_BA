...
(async () => {
  ...
  // conjunto de perguntas diferente (67 a 78 colunas na base UTD, run
    ...
    for (const alvo of alvos) {
      ...
      try {
        ...
        if (saida) {
          ...
          // Padroniza no layout da base antes de comparar e subir —
          ...
          // BOOTSTRAP: enquanto layout.colunas estiver vazio, sobe o export como
          // veio (ver comentario do layout.json). Depois de `npm run gerar-layout`
          // este caminho passa a reprojetar igual ao robo diario.
          const origem = parseCsv(r.buffer);
          let destino = { header: origem.header, rows: origem.rows, anexadas: [] };
          if (!layout.colunas.length) {
            console.warn(`[backfill] layout.json VAZIO (bootstrap): ${alvo.arquivo} sobe com as ${origem.header.length} colunas do export.`);
          } else {
            destino = reprojetar(layout.colunas, origem);
          ...
          const bufferFinal = layout.colunas.length
            ? Buffer.from(serializeCsv(destino.header, destino.rows), "utf8")
            : r.buffer;
