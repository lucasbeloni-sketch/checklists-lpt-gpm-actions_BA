...
// Mesma tela do repo irmao checklists-visita-previa-gpm-actions_BA (que exporta
// a UTD); aqui o Tipo de Checklist e a LPT. Passos:
...
//   -> Tipo de Checklist = "LPT - Visita Previa-BA"  <-- unica diferenca vs UTD
...
async function selecionarChoices(root, cfg, campo, alvo, token, tms = {}) {
  ...
  // LPT o token "LPT - Visita" ja e discriminante, mas o clique por texto exato
  // e o que garante que "Validação- Clientes - LPT" nunca seja pego por engano.
...
async function baixarChecklists(page, cfg, mesAno, intervalo) {
  ...
  console.log(`\n=== Checklists Pergunta/Resposta (LPT BA) | ${vInicio} a ${vFim} | arquivo ${mesAno}.csv ===`);
